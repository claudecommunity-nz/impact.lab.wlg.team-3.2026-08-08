...
def client():
    ...
    An explicit key from triage/.env wins. Failing that, fall back to the SDK's
    own credential resolution, which also picks up an `ant auth login` profile
    from ~/.config/anthropic — that is how this machine is authenticated, and
    requiring a pasted key would mean no key at all.
    """
    global _client
    if _client is None:
        key = env.api_key()
        if key:
            ...
            try:
                ...
                # Constructing succeeds with no credentials at all; make it
                # prove it can reach the API before we call it a client.
                _client.models.retrieve(MODEL)
            except Exception as exc:
                _client = None
                raise RuntimeError(
                    "No Anthropic credentials. Either put CLAUDE_API_KEY=... "
                    "in triage/.env, export ANTHROPIC_API_KEY, or run "
                    f"`ant auth login`. ({exc})") from exc
    return _client
...
def status(cfg: dict) -> dict:
    ...
    # No early return on a missing .env key: the SDK also resolves an
    # `ant auth login` profile, so let the probe below decide.
    try:
