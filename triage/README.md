...
| | |
|---|---|
| **Triage** | Sorts each report into *awareness*, *verification required*, or *action required* |
...
| **Sit rep** | *(not built yet)* Draft a situational report from themes derived across the whole board, traceable back to the report IDs behind each claim |
...
| **Set a time due** | *(not built yet)* Attach a deadline to an incident so it ages and escalates against the clock. Obligations already carry due times and countdowns; reports do not |
...
partner agencies each get an *adapter* in `triage/config/sources.yaml` that maps their
...
into the content — so the UI can always show *what came in* separately from *what we think
about it*.
